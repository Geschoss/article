function init() {
      ...
      temp.innerHTML = `
       ...
       ">
         <div style="text-align: center;">
           It doesn't appear your browser supports ${
  ...
  function reportJSError(url, lineNo, colNo, msg) {
    try {
      const { origUrl, actualLineNo } =
        window.parent.getActualLineNumberAndMoveTo(url, lineNo, colNo)
      url = origUrl
      lineNo = actualLineNo
    } catch (ex) {
  ...
  let setupLesson = function (canvas, opt_options) {
    ...
    if (isInIFrame()) {
      ...
      if (
        titleElem &&
        titleElem.getAttribute('addtitletodoc') !== 'false'
      ) {
        ...
        if (ctx && ctx.bindTexture) {
          ctx = makeDebugContext(ctx, {
            ...
            errorFunc: function (err, funcName, args) {
              ...
              const enumedArgs = [].map.call(
                args,
                function (arg, ndx) {
                  let str = glFunctionArgToString(
                    funcName,
                    numArgs,
                    ndx,
                    arg
              ...
              const errorMsg = `WebGL error ${glEnumToString(
                err
              )} in ${funcName}(${enumedArgs.join(', ')})`
...
export const lessonsHelper = init()
